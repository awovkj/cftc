...
async function validateDatabaseStructure(config) {
  try {
    const tables = ['categories', 'user_settings', 'files', 'upload_sessions', 'file_chunks'];
    ...
    const tableStructures = {
      ...
      upload_sessions: [
        ...
        { name: 'original_file_type', type: 'TEXT' },
...
export default {
  async fetch(request, env) {
    ...
    const routes = {
      ...
      '/chunk/init': () => handleChunkInit(request, config),
      '/chunk/part': () => handleChunkPart(request, config),
      '/chunk/status': () => handleChunkStatus(request, config),
      '/chunk/complete': () => handleChunkComplete(request, config),
      '/chunk/cleanup': () => handleChunkCleanup(request, config),
      '/webhook': () => { 
...
// ===== 分片上传后端接口 =====
async function handleChunkInit(request, config) {
  try {
    if (config.enableAuth && !authenticate(request, config)) {
    ...
    let originalFileName, originalFileType, totalChunks, storageType, categoryId;
    ...
    if (contentType.includes('application/json')) {
      ...
      storageType = body.storageType || 'telegram';
      ...
      storageType = formData.get('storageType') || 'telegram';
      categoryId = formData.get('categoryId') || null;
    }
    if (!originalFileName || !originalFileType || !totalChunks || isNaN(totalChunks)) {
      return new Response(JSON.stringify({ status: 0, msg: '初始化参数缺失' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    const uploadId = `up_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const chatId = (config.tgChatId && config.tgChatId.length) ? config.tgChatId[0] : null;
    ...
    return new Response(JSON.stringify({ status: 1, uploadId }), { headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
    console.error('[Chunk Init Error]', error);
    return new Response(JSON.stringify({ status: 0, msg: '初始化失败', error: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
async function handleChunkPart(request, config) {
  try {
    if (config.enableAuth && !authenticate(request, config)) {
      return new Response(JSON.stringify({ status: 0, msg: '未授权' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (!file || !uploadId || isNaN(chunkIndex)) {
      return new Response(JSON.stringify({ status: 0, msg: '分片参数缺失' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    const session = await config.database.prepare('SELECT * FROM upload_sessions WHERE upload_id = ?').bind(uploadId).first();
    if (!session) {
      return new Response(JSON.stringify({ status: 0, msg: '会话不存在或已过期' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    }
    const storageType = session.storage_type || 'telegram';
    let storedFileId = null;
    let storedMessageId = null;
    if (storageType === 'r2') {
      if (!config.bucket) {
        return new Response(JSON.stringify({ status: 0, msg: 'R2未配置' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
      }
      const key = `chunks/${uploadId}/${String(chunkIndex).padStart(5, '0')}.part`;
      await config.bucket.put(key, await file.arrayBuffer(), { httpMetadata: { contentType: file.type || 'application/octet-stream' } });
      storedFileId = key;
      storedMessageId = -1;
    } else {
      const tgFormData = new FormData();
      tgFormData.append('chat_id', config.tgStorageChatId);
      tgFormData.append('document', file, `${uploadId}.${String(chunkIndex).padStart(5, '0')}.part`);
      tgFormData.append('caption', `Chunk ${chunkIndex + 1}/${session.total_chunks} • ${session.original_file_name} • ${uploadId}`);
      const tgResponse = await fetch(`https://api.telegram.org/bot${config.tgBotToken}/sendDocument`, { method: 'POST', body: tgFormData });
      if (!tgResponse.ok) {
        const errText = await tgResponse.text();
        throw new Error('发送分片到Telegram失败: ' + errText);
      ...
      const tgData = await tgResponse.json();
      storedMessageId = tgData.result?.message_id || null;
      storedFileId = tgData.result?.document?.file_id || null;
      if (!storedFileId || !storedMessageId) {
        throw new Error('未获取到分片file_id或消息ID');
      }
    }
    ...
    return new Response(JSON.stringify({ status: 1, uploadId, chunkIndex }), { headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
    console.error('[Chunk Part Error]', error);
    return new Response(JSON.stringify({ status: 0, msg: '分片上传失败', error: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
async function handleChunkStatus(request, config) {
  try {
    if (config.enableAuth && !authenticate(request, config)) {
      return new Response(JSON.stringify({ status: 0, msg: '未授权' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (!uploadId) {
      return new Response(JSON.stringify({ status: 0, msg: '缺少uploadId' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    const session = await config.database.prepare('SELECT * FROM upload_sessions WHERE upload_id = ?').bind(uploadId).first();
    if (!session) {
      return new Response(JSON.stringify({ status: 0, msg: '会话不存在或已过期' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    }
    const chunks = await config.database.prepare('SELECT chunk_index, status, size, message_id, file_id FROM file_chunks WHERE upload_id = ? ORDER BY chunk_index ASC').bind(uploadId).all();
    const results = chunks.results || [];
    const completed = results.filter(c => c.status === 'completed').length;
    return new Response(JSON.stringify({ status: 1, uploadId, totalChunks: session.total_chunks, completed, chunks: results }), { headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
    console.error('[Chunk Status Error]', error);
    return new Response(JSON.stringify({ status: 0, msg: '查询失败', error: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
async function handleChunkComplete(request, config) {
  try {
    if (config.enableAuth && !authenticate(request, config)) {
      return new Response(JSON.stringify({ status: 0, msg: '未授权' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    const chunks = await config.database.prepare('SELECT chunk_index, file_id, message_id, size FROM file_chunks WHERE upload_id = ? ORDER BY chunk_index ASC').bind(uploadId).all();
    const list = chunks.results || [];
    if (list.length !== session.total_chunks) {
      return new Response(JSON.stringify({ status: 0, msg: `分片数量不匹配: 期望 ${session.total_chunks}, 实际 ${list.length}` }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    const ext = (session.original_file_name || '').split('.').pop() || 'bin';
    const finalUrl = `https://${config.domain}/${Date.now()}.${ext}`;
    const totalSize = list.reduce((sum, c) => sum + (c.size || 0), 0);
    ...
    `).bind(
      ...
      (session.storage_type === 'r2' ? 'r2_chunk' : 'telegram_chunk'),
      session.category_id,
      session.chat_id
    ).run();
    ...
    return new Response(JSON.stringify({ status: 0, msg: '合并完成失败', error: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
async function handleChunkCleanup(request, config) {
  try {
    if (config.enableAuth && !authenticate(request, config)) {
      return new Response(JSON.stringify({ status: 0, msg: '未授权' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    }
    const body = await request.json();
    const uploadId = body.uploadId;
    const deleteMessages = !!body.deleteMessages;
    if (!uploadId) {
      return new Response(JSON.stringify({ status: 0, msg: '缺少uploadId' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    const chunks = await config.database.prepare('SELECT message_id, file_id FROM file_chunks WHERE upload_id = ?').bind(uploadId).all();
    const list = chunks.results || [];
    if (deleteMessages) {
      for (const c of list) {
        if (c.message_id && c.message_id > 0) {
          try {
            await fetch(`https://api.telegram.org/bot${config.tgBotToken}/deleteMessage?chat_id=${config.tgStorageChatId}&message_id=${c.message_id}`);
          } catch (e) {
            console.warn('删除分片消息失败:', e.message);
          }
        }
      }
    }
    await config.database.prepare('DELETE FROM file_chunks WHERE upload_id = ?').bind(uploadId).run();
    await config.database.prepare('DELETE FROM upload_sessions WHERE upload_id = ?').bind(uploadId).run();
    return new Response(JSON.stringify({ status: 1, msg: '清理完成', uploadId }), { headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
    console.error('[Chunk Cleanup Error]', error);
    return new Response(JSON.stringify({ status: 0, msg: '清理失败', error: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
